...
- **`remote`** —— `delivery.sh` 在 Step 4 用 `git -C <target> remote get-url origin`
  探测 **被分析项目** 的 remote（不是 workspace、不是 skill）。SSH URL 自动转 https，
  尾巴 `.git` 剥掉；provider 由 host 推断（github / gitlab / bitbucket / unknown）；
  branch 用 `git rev-parse --abbrev-ref HEAD`，HEAD 解到 detached 时退化为 `"main"`。
  探测不到任何 remote 时写 `"remote": null`。`<SourcePointers>` 组件读这个字段拼
  GitHub / GitLab / Bitbucket 可点链接（拼接规则见 `references/source-pointers.md` §5）。
